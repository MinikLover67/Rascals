...
# Web auto-login: copy the desktop-published login (if any) into the served
# dir so browsers on this PC sign in as you with zero clicks.
WEBLINK="\$HOME/.local/share/com.rascals.chat/weblink.json"
if [ -f "\$WEBLINK" ]; then cp -f "\$WEBLINK" "\$DIR/rascals-account.json"
else rm -f "\$DIR/rascals-account.json"; fi
